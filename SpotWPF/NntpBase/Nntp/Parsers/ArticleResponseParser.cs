...
//            log.LogInformation("Start parsing headers");
            while (enumerator.MoveNext())
            {
                string line = enumerator.Current;
//                log.LogInformation("Process line: {line}", line);
...
//                log.LogInformation("Process header key: {key}, value: {value}", header.Key, header.Value);
                if (header.Key == "X-XML") {
                    lValue = "(" + lCount.ToString() + ")" + header.Value;
                    lCount++;
                } else {
                    lValue = header.Value;
                }
                dict.Add(header.Key, lValue);
            }
            return dict;
        }
